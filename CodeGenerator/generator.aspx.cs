...
{
    const int PROPERTY_NESTED_LEVELS = 5; // currently max levels of nested properties is five
    ...
    Hashtable _propertyTypeMappings; // maps HighChart types to C# types, for specific properties that cannot be autogenerated
    ...
    {
        _defaultValueMappings.Add("Shadow", "new Shadow() { Enabled = false }");
        _defaultValueMappings.Add("PlotShadow", "new Shadow() { Enabled = false }");
        _defaultValueMappings.Add("Animation", "new Animation() { Enabled = true }");
    ...
    public string MapDefaultValue(ApiItem item)
    {
        ...
        else
        {
            ...
            if (_defaultValueMappings[FirstCharToUpper(item.Title)] != null)
            {
                return _defaultValueMappings[FirstCharToUpper(item.Title)].ToString();
            }
