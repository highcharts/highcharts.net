...
/// <summary>
/// Summary description for HighmapsAspNetMvc
/// </summary>
public class HighmapsAspNetMvc
{
    const int PROPERTY_NESTED_LEVELS = 10; // currently max levels of nested properties is five
    const string ROOT_CLASS = "Highmaps"; // the name of the root class
    const string ROOT_NAMESPACE = "Maps"; // the name of the root class
    ...
    public HighmapsAspNetMvc(HttpServerUtility server)
    {
    ...
    private void GenerateClass(ApiItem item)
    {
        ...
        {
            fileName = Server.MapPath("~/CodeGeneration/" + ROOT_CLASS + "/_Unknown.cs");

            File.AppendAllText(fileName, codeTemplate);
        }
    ...
    private void InitSeriesMappings()
    {
        ...
        // Highmaps specific
        _seriesMappings.Add("series<map>", "MapSeries");
        _seriesMappings.Add("series<mappoint>", "MapPointSeries");
        _seriesMappings.Add("series<mapline>", "MapLineSeries");
        _seriesMappings.Add("series<mapbubble>", "MapBubbleSeries");
    }
