...
namespace Highsoft.Web.Mvc.Maps
{
	public partial class ChartEvents  : BaseObject
	{
		...
		/// <summary>
		/// <p>Fires when a drilldown point is clicked, before the new series is added. Event arguments include <code>point</code> for the originating point, <code>seriesOptions</code> for the new series and <code>category</code> to indicate whether a category label was clicked.</p><p>This event is also utilized for async drilldown, where the seriesOptions are not added by option, but rather loaded async.</p>
		/// </summary>
		...
		internal override Hashtable ToHashtable()
		{
			...
			if (AddSeries != AddSeries_DefaultValue) { h.Add("addSeries",AddSeries); Highmaps.AddFunction("ChartEventsAddSeries.addSeries", AddSeries); }  
			if (AfterPrint != AfterPrint_DefaultValue) { h.Add("afterPrint",AfterPrint); Highmaps.AddFunction("ChartEventsAfterPrint.afterPrint", AfterPrint); }  
			if (BeforePrint != BeforePrint_DefaultValue) { h.Add("beforePrint",BeforePrint); Highmaps.AddFunction("ChartEventsBeforePrint.beforePrint", BeforePrint); }  
			if (Click != Click_DefaultValue) { h.Add("click",Click); Highmaps.AddFunction("ChartEventsClick.click", Click); }  
			if (Drilldown != Drilldown_DefaultValue) { h.Add("drilldown",Drilldown); Highmaps.AddFunction("ChartEventsDrilldown.drilldown", Drilldown); }  
			if (Drillup != Drillup_DefaultValue) { h.Add("drillup",Drillup); Highmaps.AddFunction("ChartEventsDrillup.drillup", Drillup); }  
			if (Load != Load_DefaultValue) { h.Add("load",Load); Highmaps.AddFunction("ChartEventsLoad.load", Load); }  
			if (Redraw != Redraw_DefaultValue) { h.Add("redraw",Redraw); Highmaps.AddFunction("ChartEventsRedraw.redraw", Redraw); }  
			if (Selection != Selection_DefaultValue) { h.Add("selection",Selection); Highmaps.AddFunction("ChartEventsSelection.selection", Selection); }  
