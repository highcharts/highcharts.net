...
namespace Highsoft.Web.Mvc.Stocks
{
	public partial class AreasplineSeries  : Series
	{
		public AreasplineSeries()
		{
			...
			Type = Type_DefaultValue = AreasplineSeriesType.Null;
		...
		/// <summary>
		/// An array of data points for the series. For the `areaspline` seriestype, points can be given in the following ways:1.  An array of numerical values. In this case, the numerical valueswill be interpreted as `y` options. The `x` values will be automaticallycalculated, either starting at 0 and incremented by 1, or from `pointStart`and `pointInterval` given in the series options. If the axis hascategories, these will be used. Example: ```js data: [0, 5, 3, 5] ```2.  An array of arrays with 2 values. In this case, the values correspondto `x,y`. If the first value is a string, it is applied as the nameof the point, and the `x` value is inferred. ```js    data: [        [0, 10],        [1, 9],        [2, 3]    ] ```3.  An array of objects with named values. The following snippet shows only afew settings, see the complete options set below. If the total number of datapoints exceeds the series'[turboThreshold](#series.areaspline.turboThreshold), this option is notavailable. ```js    data: [{        x: 1,        y: 4,        name: "Point2",        color: "#00FF00"    }, {        x: 1,        y: 4,        name: "Point1",        color: "#FF00FF"    }] ```
		/// </summary>
		...
		internal override Hashtable ToHashtable()
		{
			...
			if (PointDescriptionFormatter != PointDescriptionFormatter_DefaultValue) { h.Add("pointDescriptionFormatter",PointDescriptionFormatter); Highstock.AddFunction("AreasplineSeriesPointDescriptionFormatter.pointDescriptionFormatter", PointDescriptionFormatter); }  
