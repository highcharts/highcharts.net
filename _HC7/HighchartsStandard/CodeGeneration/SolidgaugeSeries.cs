...
namespace Highsoft.Web.Mvc.Charts
{
	public partial class SolidgaugeSeries  : Series
	{
		...
		/// </summary>
		public SolidgaugeSeriesLegendSymbol LegendSymbol { get; set; }
		private SolidgaugeSeriesLegendSymbol LegendSymbol_DefaultValue { get; set; }
		...
		/// </summary>
		public SolidgaugeSeriesOnPoint OnPoint { get; set; }
		private SolidgaugeSeriesOnPoint OnPoint_DefaultValue { get; set; }
		...
		/// <summary>
		/// Whether to draw rounded edges on the gauge. This options adds the radiusof the rounding to the ends of the arc, so it extends past the actualvalues. When `borderRadius` is set, it takes precedence over `rounded`. A`borderRadius` of 50% behaves like `rounded`, except the shape is notextended past its value.
		/// </summary>
		...
		/// </summary>
		public SolidgaugeSeriesSonification Sonification { get; set; }
		private SolidgaugeSeriesSonification Sonification_DefaultValue { get; set; }
		...
		internal override Hashtable ToHashtable(Highcharts highcharts)
		{
			...
			if (Cursor != SolidgaugeSeriesCursor.Null) h.Add("cursor", highcharts.FirstCharacterToLower(Cursor.ToString()));
			...
			if (LegendSymbol != SolidgaugeSeriesLegendSymbol.Null) h.Add("legendSymbol", highcharts.FirstCharacterToLower(LegendSymbol.ToString()));
			if (Linecap != SolidgaugeSeriesLinecap.Null) h.Add("linecap", highcharts.FirstCharacterToLower(Linecap.ToString()));
			...
			if (PointIntervalUnit != SolidgaugeSeriesPointIntervalUnit.Null) h.Add("pointIntervalUnit", highcharts.FirstCharacterToLower(PointIntervalUnit.ToString()));
			...
			if (Rounded != null) h.Add("rounded",Rounded);
