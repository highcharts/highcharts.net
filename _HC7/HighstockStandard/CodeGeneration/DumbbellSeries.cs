...
namespace Highsoft.Web.Mvc.Stocks
{
	public partial class DumbbellSeries  : Series
	{
		...
		/// Possible values: `"on"`, `"between"`, `number`.In a column chart, when pointPlacement is `"on"`, the point will notcreate any padding of the X axis. In a polar column chart this meansthat the first column points directly north. If the pointPlacement is`"between"`, the columns will be laid out between ticks. This isuseful for example for visualising an amount between two points intime or in a certain sector of a polar chart.Since Highcharts 3.0.2, the point placement can also be numeric,where 0 is on the axis value, -0.5 is between this value and theprevious, and 0.5 is between this value and the next. Unlike thetextual options, numeric point placement options won't affect axispadding.Note that pointPlacement needs a [pointRange](#plotOptions.series.pointRange) to work. For column series this iscomputed, but for line-type series it needs to be set.For the `xrange` series type and gantt charts, if the Y axis is acategory axis, the `pointPlacement` applies to the Y axis rather thanthe (typically datetime) X axis.Defaults to `undefined` in cartesian charts, `"between"` in polarcharts.
		/// </summary>
		public DumbbellSeriesPointPlacement PointPlacementNumber { get; set; }
		...
		internal override Hashtable ToHashtable(Highstock highstock)
		{
			...
			if (Compare != DumbbellSeriesCompare.Null) h.Add("compare", highstock.FirstCharacterToLower(Compare.ToString()));
			if (CompareBase != DumbbellSeriesCompareBase.Null) h.Add("compareBase", highstock.FirstCharacterToLower(CompareBase.ToString()));
			...
			if (Cursor != DumbbellSeriesCursor.Null) h.Add("cursor", highstock.FirstCharacterToLower(Cursor.ToString()));
			if (Custom != null) h.Add("custom",Custom);
			if (DashStyle != DumbbellSeriesDashStyle.Null) h.Add("dashStyle", highstock.FirstCharacterToLower(DashStyle.ToString()));
			...
			if (FindNearestPointBy != DumbbellSeriesFindNearestPointBy.Null) h.Add("findNearestPointBy", highstock.FirstCharacterToLower(FindNearestPointBy.ToString()));
			if (GapSize != null) h.Add("gapSize",GapSize);
			if (GapUnit != DumbbellSeriesGapUnit.Null) h.Add("gapUnit", highstock.FirstCharacterToLower(GapUnit.ToString()));
			...
			if (LegendSymbol != DumbbellSeriesLegendSymbol.Null) h.Add("legendSymbol", highstock.FirstCharacterToLower(LegendSymbol.ToString()));
			if (Linecap != DumbbellSeriesLinecap.Null) h.Add("linecap", highstock.FirstCharacterToLower(Linecap.ToString()));
			...
			if (PointIntervalUnit != DumbbellSeriesPointIntervalUnit.Null) h.Add("pointIntervalUnit", highstock.FirstCharacterToLower(PointIntervalUnit.ToString()));
			if (PointPadding != null) h.Add("pointPadding",PointPadding);
			if (PointPlacement != DumbbellSeriesPointPlacement.Null) h.Add("pointPlacement", highstock.FirstCharacterToLower(PointPlacement.ToString()));
			if (PointPlacementNumber != DumbbellSeriesPointPlacement.Null) h.Add("pointPlacement", highstock.FirstCharacterToLower(PointPlacementNumber.ToString()));
			...
			if (Step != DumbbellSeriesStep.Null) h.Add("step", highstock.FirstCharacterToLower(Step.ToString()));
