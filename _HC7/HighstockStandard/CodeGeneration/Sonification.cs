...
namespace Highsoft.Web.Mvc.Stocks
{
	public partial class Sonification  : BaseObject
	{
		...
		internal override Hashtable ToHashtable(Highstock highstock)
		{
			...
			if (GlobalContextTracks.IsDirty(highstock)) h.Add("globalContextTracks",GlobalContextTracks.ToHashtable(highstock));
			if (GlobalTracks.IsDirty(highstock)) h.Add("globalTracks",GlobalTracks.ToHashtable(highstock));
			if (MasterVolume != MasterVolume_DefaultValue) h.Add("masterVolume",MasterVolume);
			if (Order != Order_DefaultValue) h.Add("order", highstock.FirstCharacterToLower(Order.ToString()));
